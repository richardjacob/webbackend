...
class AuthViaEmail implements AuthInterface
{
	...
	public function validate(Request $request)
	{
		if ($request->referral_code != '') {
			...
			if (is_object($hub_employee)) return false;
		}
		...
		if ($user_count > 0) {
			Log::warning("You Have Alrady Account Please Login");
			return response()->json([
				'status_code'     => '0',
		...
		// 	$user_count_nid = User::where('nid_number', $request->nid_number)->where('nid_number', '!=', '')->where('user_type', $request->user_type)->count();
		// 	if ($user_count_nid > 0) {
		// 		Log::warning("You Have Alrady Account Please Login");
		// 		return response()->json([
		// 			'status_code'     => '0',
		...
		$user_email_count = User::where('email', $request->email_id)
			->where('email', '!=', '')
			// ->whereNotNull('email')
			->where('user_type', $request->user_type)->count();
	 ...
	 */
	public function create(Request $request, $unique_id = "")
	{
		if ($unique_id == "") {
		...
		$user_count = User::where('mobile_number', $request->mobile_number)->where('user_type', $request->user_type)->count();
		$user_count_nid = 0;
		...
		if ($user_count == 0 AND $user_count_nid == 0) {
			$user = new User;
			...
			return User::where('mobile_number', $request->mobile_number)->where('user_type', $request->user_type)->first();
		}
	}
