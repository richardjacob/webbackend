	...
	public function boot()
	{
		...
		if (env('DB_DATABASE') != '') {
			...
			if ($has_site_settings) {
				...
				$site_settings = Cache()->remember('site_settings', Config('cache.one_year'), function () {
			...
			$has_country = Cache()->remember('has_country', Config('cache.one_year'), function () {
			...
			$has_car_type = Cache()->remember('has_car_type', Config('cache.one_year'), function () {
				return Schema::hasTable('car_type');
			});
			if ($has_car_type) {
				//if (Schema::hasTable('car_type')) {
				$car_type = Cache()->remember('car_type', Config('cache.one_year'), function () {
					return CarType::where('status', 'Active')->get();
				});
			...
			$has_api_credentials = Cache()->remember('has_api_credentials', Config('cache.one_year'), function () {
				return Schema::hasTable('api_credentials');
			});
			...
			$has_admin = Cache()->remember('has_admin', Config('cache.one_year'), function () {
				return Schema::hasTable('admin');
			});
			if ($has_admin) {
			...
			$has_supports = Cache()->remember('has_supports', Config('cache.one_year'), function () {
				return Schema::hasTable('supports');
			});
			if ($has_supports) {
				//if(Schema::hasTable('supports')) {

				$support = Cache()->remember('support', Config('cache.one_year'), function () {
					return Support::active()->get();
				});
			...
			$has_payment_gateway = Cache()->remember('has_payment_gateway', Config('cache.one_year'), function () {
				return Schema::hasTable('payment_gateway');
			});
			...
			$has_email_settings = Cache()->remember('has_email_settings', Config('cache.one_year'), function () {
				return Schema::hasTable('email_settings');
			});
			if ($has_email_settings)
			//if(Schema::hasTable('email_settings'))
			{
				$result = Cache()->remember('email_settings', Config('cache.one_year'), function () {
					return DB::table('email_settings')->get();
				});
	...
	protected function bindModels()
	{
		...
		if ($has_email_settings) {
			...
			$this->app->singleton('email_settings', function ($app) {
				$email_settings = Cache()->remember('email_settings', Config('cache.one_year'), function () {
		...
		if ($has_api_credentials) {
			...
			$this->app->singleton('api_credentials', function ($app) {
				$api_credentials = Cache()->remember('api_credentials', Config('cache.one_year'), function () {
					return ApiCredentials::get();
				});
		...
		if ($has_payment_gateway) {
			...
			$this->app->singleton('payment_gateway', function ($app) {
				...
				$payment_gateway = Cache()->remember('payment_gateway', Config('cache.one_year'), function () {
		...
		$has_referral_settings = Cache()->remember('has_referral_settings', Config('cache.one_year'), function () {
			return Schema::hasTable('referral_settings');
		});
		if ($has_referral_settings) {
			//if (Schema::hasTable('referral_settings')) {			
			$this->app->singleton('referral_settings', function ($app) {
				$referral_settings = Cache()->remember('referral_settings', Config('cache.one_year'), function () {
					return ReferralSetting::get();
				});
		...
		$has_fees = Cache()->remember('has_fees', Config('cache.one_year'), function () {
			return Schema::hasTable('fees');
		});
		if ($has_fees) {
			//if (Schema::hasTable('fees')) {
			$this->app->singleton('fees', function ($app) {

				$fees = Cache()->remember('fees', Config('cache.one_year'), function () {
					return Fees::get();
				});
		...
		$has_vehicle_type = Cache()->remember('has_vehicle_type', Config('cache.one_year'), function () {
			return Schema::hasTable('vehicle_type');
		});
		if ($has_vehicle_type) {
			//if (Schema::hasTable('vehicle_type')) {
			$this->app->singleton('vehicle_type', function ($app) {
				$car_types = Cache()->remember('car_types', Config('cache.one_year'), function () {
					return \App\Models\CarType::get();
				});
