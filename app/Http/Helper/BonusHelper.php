...
class BonusHelper
{
	public function transfer_to_balance($user, $bonus)
	{
		if ($bonus->withdrawal_method == 'Balance') { // wallet
			...
			if (!$exist_driver_bonus) {
				...
				$driver_bonus->company_id = $user->company_id;
	...
	public function bonus_status_v3($user_id, $bonus_type, $page = 1)
	{
		...
		foreach ($bonuses as $bonus) {
			...
			if ($bonus->bonus_type == 'DriverOnlineBonus') {
				...
				$bonus_end_date = date("d M Y", strtotime($bonus->end_date));
				if (strtotime($curent_date) <= strtotime($bonus_end_date)) {
					$is_current_week = "true";
				}

				$your_progress = array();
				$earning = 0;
				if ($completed_trips > $bonus->first_stage_min_trips - 1 and $completed_trips <= $bonus->first_stage_max_trips) {
					$earning = $bonus->first_stage_amount;
				} else if ($completed_trips > $bonus->second_stage_min_trips - 1 and $completed_trips <= $bonus->second_stage_max_trips) {
					$earning = $bonus->second_stage_amount;
				} else if ($completed_trips > $bonus->third_stage_min_trips - 1 and $completed_trips <= $bonus->third_stage_max_trips) {
					$earning = $bonus->third_stage_amount;
				}


				$bonus_stage[] =  array(
					'required_ride' => "$bonus->first_stage_min_trips" . " Ride",
					'bonus_amount' => "$bonus->first_stage_amount",
				);
				$bonus_stage[] =  array(
					'required_ride' => "$bonus->second_stage_min_trips" . " Ride",
					'bonus_amount' => "$bonus->second_stage_amount",
				);
				$bonus_stage[] =  array(
					'required_ride' => "$bonus->third_stage_min_trips" . " Ride",
					'bonus_amount' => "$bonus->third_stage_amount",
				);


				$your_progress[] =  array(
					'title' => "Trip Completed",
					'progress' => "$completed_trips" . "/" . "$number_of_trips",
					'progress_bar_status' => (int)$rate,
					'is_progress_bar' => "true"
			...
			} else if ($bonus->bonus_type == 'DriverSignupBonus') {
				$bonus_start_date = date("d M Y", strtotime($bonus->bonus_date));
				$number_of_days = $bonus->number_of_days - 1;
				$bonus_end_date = date('d M Y', strtotime($bonus->bonus_date . ' + ' . $number_of_days . ' days'));
				if (strtotime(date('Y-m-d')) > strtotime(date('Y-m-d', strtotime($bonus_end_date)))) {
					$remaining_days = 0;
				} else {
					$timeleft = strtotime(date('Y-m-d', strtotime($bonus_end_date))) - strtotime(date('Y-m-d'));
					$remaining_days = $timeleft / (60 * 60 * 24);
				}
				...
				$bonus_array =  array(
					'bonus_id' => $bonus->id,
					'bonus_date' =>  $bonus_start_date . " - " . $bonus_end_date,
	...
	// public function add_update_online_bonus_v2($driver_id)
	// { 
	// 	$today = date('Y-m-d');
	// 	$yesterday = date("Y-m-d", strtotime("-1 days"));


	// 	$activity = Activity::whereDate('created_at', '=', $yesterday)
	// 							 ->where('user_id', $driver_id)
	// 							 ->select(
	// 							 	'user_id',
	// 								DB::raw("DATE_FORMAT(created_at, '%Y-%m-%d') as online_date"),
	...
	// 			$online_bonus->bonus_date = $today;
	// 			$online_bonus->unique_id = $today . "_" . $driver_id . "_OnlineBonus";
	// 			$online_bonus->create_method = 'BonusHelper::add_update_online_bonus_v2';
	// 			try {
	// 				$online_bonus->save();
	// 			} catch (\Exception $e) {
	// 				return false;
	...
	// public function adjust_driver_online_bonus_v2($driver_id)
	// { // by Cron setup
	// 	$online_date = date('Y-m-d', strtotime("-1 days"));
	// 	$bonus = Bonus::where('user_type', 'Driver')
	// 				->where('bonus_type', 'DriverOnlineBonus')
