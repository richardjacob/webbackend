...
class EmailController extends Controller
{
    ...
    public function send_email(Request $request)
    {
        if($request->isMethod('GET')) {
            ...
            // if($request->message_priority == 'schedule') {
            ...
            if($request->message_priority == 'schedule') {
                // insert into db
                $table = new ScheduleEmail;
                $table->schedule_time = str_replace('/','-', $request->schedule_time);
                $table->url = $data['url'];
                $table->locale =  $data['locale'];
                $table->view_file =  'emails.custom_email';
                $table->subject = $request->subject;
                $table->message = $request->message;
                $table->email = json_encode($emails);
                $table->save();
                $this->helper->flash_message('success', 'Schedule Email Saved Successfully');
    ...
    public function cron_jobs()
    {
        ...
        $now = '2021-05-29 21:00:00'; // to be off
        $schedule_messages = ScheduleEmail::where('schedule_time', $now)
                                                ->where('status', '0')
                                                ->get();

        $data['url']    = url('/').'/';
        foreach($schedule_messages as $sm){
            $data['locale'] = $sm->locale;
            $url = $sm->url;
            $view_file = $sm->view_file;
            $subject = $sm->subject;
            $message = $sm->message;
            $email = $sm->email;
            $emails = json_decode($sm->email);
