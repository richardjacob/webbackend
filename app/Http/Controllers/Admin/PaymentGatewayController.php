...
class PaymentGatewayController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->nagad_enabled) {
            $nagad_rules = array(
                'nagad_mode'           => 'required',
                'nagad_merchant_id'    => 'required',
                'nagad_account_number'    => 'required',
                'nagad_public_key'     => 'required',
                'nagad_private_key'    => 'required',
            );
        }
        //$rules = array_merge($paypal_rules,$stripe_rules,$bt_rules,$nagad_rules);
        $rules = array_merge($nagad_rules);
        ...
        $attributes = array(
            ...
            // 'nagad_mode'           => 'Payment Mode',
            'nagad_merchant_id'    => 'Merchant ID',
            'nagad_account_number' => 'Account Number',
        ...
        if($request->nagad_enabled == '0' && $request->bkash_enabled == '0' && $request->rocketenabled == '0') {
            flashMessage('danger', 'Please enable atleast One Payment Gateway');
            return back();
        }
        ...
        //added by Taslim
        PaymentGateway::where(['name' => 'is_enabled', 'site' => 'Nagad'])->update(['value' => $request->nagad_enabled]);
        PaymentGateway::where(['name' => 'mode', 'site' => 'Nagad'])->update(['value' => $request->nagad_mode]);
        PaymentGateway::where(['name' => 'merchant_id', 'site' => 'Nagad'])->update(['value' => $request->nagad_merchant_id]);
        PaymentGateway::where(['name' => 'account_number', 'site' => 'Nagad'])->update(['value' => $request->nagad_account_number]);
        PaymentGateway::where(['name' => 'public_key', 'site' => 'Nagad'])->update(['value' => $request->nagad_public_key]);
        PaymentGateway::where(['name' => 'private_key', 'site' => 'Nagad'])->update(['value' => $request->nagad_private_key]);
        // end
