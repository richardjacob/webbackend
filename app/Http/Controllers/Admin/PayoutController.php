...
class PayoutController extends Controller
{
    ...
    public function payout_driver_balance(PayoutDriverBalanceDataTable $dataTable)
    {

        return $dataTable->setFilter('OverAll')->render('admin.payouts.driver_balance');
    }
    ...
    public function payout_per_week_report(PayoutReportsDataTable $dataTable)
    {
        ...
        $data['per_week'] = 'Per week Payouts';
    ...
    public function payout_to_driver(Request $request)
    {
        ...
        //nagad should be added
        if ($payout_details->type == 'nagad' || $payout_details->type == 'bkash' || $payout_details->type == 'rocket' || $payout_details->type == 'banktransfer') {
            $payout_data['driver_transaction_id'] = $tr_no; //for temporarily
            $payout_details->type = ucwords($payout_details->type);
        }


        $payout_service = resolve('App\Services\Payouts\\' . ucwords($payout_details->type) . 'Payout');
        ...
        $payment_data['driver_transaction_id']   = $tr_no;  //$pay_result['transaction_id'];
        $payment_data['driver_payout_status']   = isset($pay_result['is_pending']) ? 'Processing' : 'Paid';
        $payout_data['payment_date'] = Carbon::now();
        Payment::whereIn('trip_id', $trip_ids)->update($payment_data);

        DB::table('payout_transaction_history')->insert(
            [
                'driver_id' => $request->driver_id,
                'amount' => $amount,
                'transaction_id' => $tr_no,
