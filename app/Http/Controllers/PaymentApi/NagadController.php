...
class NagadController extends Controller
{
    ...
    public static function index($OrderId,$amount){
        //($mode,$merchant_id,$account_number,$public_key,$private_key,$OrderId,$amount
        $DateTime = Date('YmdHis');
        $random = self::generateRandomString();  
        $payment_gateway = (object) Config('payment_gateway.nagad'); 
       
        $mode = $payment_gateway->mode;
        $merchant_id = $payment_gateway->merchant_id; 
        $account_number = $payment_gateway->account_number; 
        $public_key = $payment_gateway->public_key; 
        $private_key = $payment_gateway->private_key; 
        $call_back_url = $payment_gateway->call_back_url; 
