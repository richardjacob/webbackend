...
class EarningController extends Controller
{
	...
	{
		Log::info("earning_chart Api Stp:1 :", $request->all());
		...
		for ($i = 0; $i < 7; $i++) {
			...
			$trips = Trips::whereRaw("DATE_FORMAT(created_at,'%Y-%m-%d') = '" . $created_at . "'")
		...
		CustomLog::info("earning_chart Api Stp:2 :");
	...
	{
		Log::info("after_payment Api Stp:1 :", $request->all());
		...
		CustomLog::info("after_payment Api Stp:2 Current Trip Status :" . $trip->status);
		...
		$push_data2['push_title'] = $push_title;
		$push_data2['data'] = array(
			'trip_payment' => array(
				'status' => 'Paid',
				'trip_id' => $trip->id,
			)
		...
		if ($user->user_type == "Driver" || $user->user_type == "driver") {
			CustomLog::info("after_payment Api Stp:3 Firebase Push Notification Send To Rider:");
			$this->request_helper->SendPushNotification($rider, $push_data2);
		}


		CustomLog::info("after_payment Api Stp:4 Firebase Push Notification Test:");
		...
		CustomLog::info("after_payment Api Stp:5 :");
		return response()->json([
			'status_code' 		=> '1',
	...
	{
		Log::info("add_wallet Api Stp:1 :", $request->all());
		...
		CustomLog::info("add_wallet Api Stp:2 :");
	...
	{
		Log::info("add_promo_code Api Stp:1 :", $request->all());
		...
		if (@$promo_code_check) {
			if (@$promo_code_date_check) {
				...
				CustomLog::info("add_promo_code Api Stp:2 :");
