...
class TokenAuthController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->ajax_controller = resolve('App\Http\Controllers\Admin\AjaxController');
    }
    ...
    {
        Log::info("getUserDetails :" . json_encode($user));
    ...
    {
        Log::info("registration_user Api Stp:1 :", $request->all());
        ...
        if ($user == false) {
            ...
            if ($user->user_type == 'driver') {
                ...
                if (trim($request->nid_number) != '') {
                    $nidRequest = new \Illuminate\Http\Request();
                    $nidRequest->setMethod('POST'); 
                    $nidRequest->request->add(['nid' => trim($request->nid_number), 'user_id' => $user->id]);
                    $this->ajax_controller->verify_nid($nidRequest);
                }
            ...
            CustomLog::info("registration_user Api Stp:2 :");
    ...
    public function numbervalidation(Request $request)
    {
        ...
        try {
            ...
            $sms_result = $this->sms_helper->send($to, $otp_message,$api_or_web="api_OTP");
            //  print_r($sms_result);
            // exit;
            $sms_result =  json_decode($sms_result, true);
            if ($sms_result['Status'] == 0) {
                $Message_ID = $sms_result['Message_ID'];
                $sms_check = $this->sms_helper->send_message_check($Message_ID);
    ...
    {
        Log::info("update_device Api Stp:1 :", $request->all());
        ...
        CustomLog::info("update_device Api Stp:2 :");
    ...
    {
        Log::info("getSessionOrDefaultCode : ");

        $currency_code = Currency::defaultCurrency()->first()->code;

        CustomLog::info("getSessionOrDefaultCode :");
    }
    ...
    public function send_verify_email(Request $request){
		...
		if ($user->email != '') {
			...
			try {
				...
				return array(
					'status_code'	=> '1',
					'status_message' => __('messages.api.verification_email_send_success').$user->email
				);

			} catch (\Exception $e) {
				return array(
					'status_code'	=> '0',
					'status_message' => __('messages.api.verification_email_send_unsuccess')
				);
			}
		}
