...
class UserController extends Controller
{
	public function __construct()
	{
		...
		$this->nid_helper = resolve('App\Http\Helper\NidHelper');
	}
			...
			if ($key == 'email_phone') {
				if (is_numeric($credential)) {
					...
					if ($data->user_type == 'Company') {
						...
						if (isset($company) && $company->status == 'Pending') {
							return ['status' => 'false', 'error' => "You account still pending. Contact with Alesha Ride
							 customer care for activated", 'success' => 'true'];
						} else {
							if ($company) {
					...
					if ($data->user_type == 'Company') {
						...
						if ($user) {
							...
							//Added by Nishat 04-06-2021
							return ['status' => 'false', 'error' => trans('messages.user.no_recognize_email', ['site' => SITE_NAME]), 'success' => 'false'];
							//Added by Nishat End 04-06-2021
				...
				if (Session::get('login_type') == 'email' || Session::get('login_type') == 'mobile_number') {
					if (is_numeric($data->email)) {
						...
						if (substr($data->email, 0, 1) == "0") {
							$data->email = substr($data->email, 1);
						}
						...
						if ($guard) {
							...
							Auth::guard('company')->logout();
							return ['status' => 'true', 'error' => '', 'success' => 'true', 'user_type' => 'Company', 'username' => $data->email, 'password' => $data->password, 'domain' => env('DOMAIN')];
						} else {
	...
	public function forgotpassword(Request $request, EmailController $email_controller)
	{
		if (is_numeric($request->email)) {
			$country_code = "880";
			$requested_mobile_number = $request->email;
			if (substr($requested_mobile_number, 0, 1) == "0") {
				$requested_mobile_number = substr($requested_mobile_number, 1);
			}
			$rules = array(
				'email' => 'regex:/[0-9]/|min:10',
			);
			$messages = array(
				'email.regex' 	=> trans('messages.user.mobile_no'),
			);

			$attributes = array(
				'email' => trans('messages.user.mobile'),
			);
			$validator = Validator::make($request->all(), $rules, $messages, $attributes);
			$validator->after(function ($validator) use ($request) {
				...
				if ($request->user_type=="Company"){
					$user = Company::where('mobile_number', $requested_mobile_number)->count();
				}else {
					$user = User::where('mobile_number', $requested_mobile_number)->where('user_type', $request->user_type)->count();
				}
				
				if (!$user) {
					$validator->errors()->add('email', trans('messages.user.no_recognize') . SITE_NAME);
				}
			});
			...
			if ($request->request_type == 'send_otp') {  //send OTP
				...
				$otp_responce = $this->otp_helper->sendOtp($request->email, $country_code);
				if ($otp_responce['status_code'] == 0) {
					$data = [
						'status_code' => 0,
						'messages' => ['email' => [$otp_responce['message']]],
					];
				...
				$check_otp_responce = $this->otp_helper->checkOtp($request->otp, $request->mobile_number, $country_code);
				return json_encode($check_otp_responce);
			} elseif ($validator->fails()) {
				return back()->withErrors($validator)->withInput(); // Form calling with
			} else {
				if ($request->user_type=="Company"){
					$user = Company::where('mobile_number', $requested_mobile_number)->first();
				}else {
					$user = User::where('mobile_number', $requested_mobile_number)
					->where('user_type', $request->user_type)
					->first();
				}
				
				$url = $email_controller->otp_forgot_password_link($user,$request->user_type);
			...
			$messages = array(
				'required'                => 'Email Or Phone Number ' . trans('messages.home.field_is_required') . '',
				'exists' => trans('messages.user.no_recognize_email') . SITE_NAME,
			);
			...
			if ($request->user_type == 'Company') {
				...
				$email_controller->company_forgot_password_link($company,$request->user_type);
				flashMessage('success', trans('messages.user.link') . $company->email);
				$url = "/signin_company";
				return json_encode(['status_code' => 5, 'url' => $url]);
				//	return redirect('signin_company');
			} else {
				$user = User::whereEmail($request->email)
					->where('user_type', $request->user_type)
					->first();
				$email_controller->forgot_password_link($user,$request->user_type);
	...
	public function company_register(Request $request)
	{
		...
		$rules = array(
			...
			'email' => 'required|email',
			'mobile_number' => 'required|numeric|regex:/[0-9]/|min:10',
			'password' => 'required|min:8|regex:/[^A-Za-z0-9]+/',
		...
		$validator->after(function ($validator) use ($request) {
			...
			//Added by Nishat 04-06-2021
			$requested_mobile_number = $request->mobile_number;
			...
			$company = Company::where('mobile_number', $requested_mobile_number)->where('country_id', $request->country_id)->count();
			if ($company) {
	...
	public function rider_register(Request $request)
	{
		...
		$validator->after(function ($validator) use ($request) {
			...
			//Added by Nishat End 04-06-2021
			$user = User::where('mobile_number', $requested_mobile_number)->where('user_type', $request->user_type)->where('country_id', $request->country_id)->count();
			if ($user) {
				$validator->errors()->add('mobile_number', trans('messages.user.mobile_no_exists'));
			}

			$user_email = User::where('email', $request->email)->where('user_type', $request->user_type)->where('email', '!=', '')->count();
			if ($user_email) {
				$validator->errors()->add('email', trans('messages.user.email_exists'));
			}
		...
		if ($request->request_type == 'send_otp') {  //send OTP
			...
			$user->unique_id    = $request->mobile_number . "_" . strtolower($request->user_type);
	...
	public function driver_register(Request $request)
	{
		if ($request->step == 'basics') {
			...
			$attributes = array(
				...
				'gender' => trans('messages.profile.gender'),
				'referral_code'   => trans('messages.referrals.referral_code'),
				'nid_passport_number'   => trans('messages.profile.nid_passport_number'),
			);
			...
			$messages = array(
				...
				'mobile_number.regex' => trans('messages.user.mobile_no'),
				'referral_code.exists' 	=> trans('messages.referrals.enter_valid_referral_code'),
				// 'password' 	=> trans('messages.referrals.enter_valid_referral_code'),
				'password.regex' => 'Your password must be at least 8 characters With Alphabet And Number',
			);
			...
			$validator->after(function ($validator) use ($request) {
				...
				if($request->nid_passport == 'nid_number'){
					$nid_response = $this->nid_helper->verify_nid($request->nid_passport_number, '-'); 
					if($nid_response != trans('messages.success') AND $nid_response != 'user_id_null'){
						$validator->errors()->add('nid_passport_number', $nid_response);
					}
				}
			...
			if ($request->request_type == 'send_otp') {  //send OTP
				...
				if ($request->referral_code != '') {
					$hub_employee = HubEmployee::where('refaral_id', $request->referral_code)->first();
					if (is_object($hub_employee)) {
						$hub_id = $hub_employee->hub_id;
				...
				$user->hub_id 	= $hub_id;
				$user->hub_employee_id 	= $hub_employee_id;
				if($request->nid_passport == 'nid_number') {
					$user->nid_number= $request->nid_passport_number;
				}
				else if($request->nid_passport == 'passport_number') $user->passport_number = $request->nid_passport_number;

				try {
					if($user->save()){
						if($request->nid_passport == 'nid_number') {
							$nid_table = Nid::where('nid', $request->nid_passport_number)->first();
	...
	public function reset_password(Request $request)
	{
		if ($request->isMethod("GET")) {
			...
			if (@$password_resets->user_type=="Company"){
				$user = Company::where('mobile_number', @$password_resets->email)
				->first();
			}else {
				$user = User::where('email', @$password_resets->email)
				->where('mobile_number', @$password_resets->email)
				->first();
			}
			...
			if ($password_resets) {
				...
				if ($password_resets->user_type=="Company"){
					$data['result'] = Company::where('mobile_number', $password_result->email)->first();

				}else {
					$data['result'] = User::where('email', $password_result->email)
					->orWhere('mobile_number', $password_result->email)
					->first();
				}
		...
		$attributes = array(
			...
			'new_password.regex' => 'Your password must be at least 8 characters With Alphabet And Number',
		...
		if ($user->user_type == 'Rider') {
			...
			$signin_company= '//'.env('ADMIN_PANEL_SUB_DOMAIN'). '.' . env('DOMAIN').'/signin_company';
			return redirect($signin_company);
		}
	...
	{
		$user = User::where('id', '10003')->first();
		// $this->bonus_helper->adjust_driver_online_bonus($user);
		//$this->bonus_helper->adjust_driver_online_bonus();
