...
class DriverPaymentDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('trip_ids', function ($trips) {
               // return '<div class="min_width">'.count(explode(',', $trips->trip_ids)).'</div>';
                return '<a href="owe_trip_list/driver/'.$trips->driver_id.'">'.count(explode(',', $trips->trip_ids)).'</a>';
            ...
            ->addColumn('first_name', function ($trips) {
                return $trips->first_name.' '.$trips->last_name;
            })
    ...
    public function query(User $model)
    {
        $owe = User::join('trips', function($join) {
                ...
                ->select('trips.id as trip_id','users.id As id', 'trips.driver_id as driver_id', DB::raw('CONCAT(users.first_name, \' \', users.last_name) AS driver_name'), 'trips.currency_code as currency_code',DB::raw("GROUP_CONCAT(trips.id) as trip_ids"),DB::raw('SUM(trips.total_fare) as total_fare'), 'users.gender')
                ->groupBy('driver_id');
        return $owe;
    }
