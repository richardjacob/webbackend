    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('trip_ids', function ($owe) {
                ...
                //return '<div class="min_width text-primary owe_trip_list_modal" style="cursor: pointer" onClick="owe_trip_list_modal(\''.$owe->company->id.'\', \''.$owe->company->name.'\', \'company\')">'.count($trips_ids).'</div>';
                return '<a href="owe_trip_list/company/'.$owe->company->id.'">'.count($trips_ids).'</a>';
            })
            ...
            ->addColumn('action', function ($owe) {
                if($owe->company_id != '1') {
                    ...
                    $paid_btn = '<form action="'.route('update_company_payment').'" method="POST" class="owe_payment"  onsubmit="return confirm(\'Are you sure to payment ?\');"> 
                                <input type="hidden" name="company_id" value="'.$owe->company_id.'">
                                <input type="hidden" name="_token" value="'.csrf_token().'">
                                <button type="submit" class="btn btn-xs btn-primary owe_payment_submit"> Paid </button>
                                </form>';
    ...
    protected function getColumns()
    {
        ...
        if(LOGIN_USER_TYPE == 'admin') {
            $columns = array(
                ...
                ['data' => 'trip_ids', 'name' => 'trip_ids', 'title' => 'Total Trip','orderable' => false, 'searchable' => false],
