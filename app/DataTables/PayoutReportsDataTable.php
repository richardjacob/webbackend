...
class PayoutReportsDataTable extends DataTable
{
    ...
    public function getDriverPayout($trips)
    {
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('driver_payout', function ($trips) {
                ...
                return currency_symbol().self::getDriverPayout($trips);
            })
            ->addColumn('action', function ($trips) {
                ...
                $payout_method_data = '';
                $payout_method = PayoutCredentials::where('user_id', $trips->driver_id)
                                                    ->where('default','yes')
                                                    ->first();
                if(is_object($payout_method)){
                    $preference = PayoutPreference::where('id', $payout_method->preference_id)
                                                        ->first();
                ...
                $account_number = DB::table('payout_preference')->where('user_id', $trips->driver_id)->pluck('account_number')->first();
                ...
                if($this->filter_type == 'day_report') {
                    ...
                    //     </form>';
                        ...
                        $payment_action='
                        <label class="btn btn-xs btn-primary" onclick="make_payout_modal(\''.$account_number.'\', 
                            \''.self::getDriverPayout($trips).'\', 
                            \'driver_trip\', 
                            \''.$trips->driver_id.'\', 
                            \''.$payout_method_data.'\',
                            \''.LOGIN_USER_TYPE.'/per_day_report/'.$trips->driver_id.'/'.request()->date.'\', 
                            \''.$payout_text.'\', 
                            \'\', 
                            \'\', 
                            \'\',
                        ...
                        //used $trips->driver_payout  instead of $trips->driver_payout  
                }
                else {
                    ...
                    $payment_action='
                        <label class="btn btn-xs btn-primary" onclick="make_payout_modal(\''.$account_number.'\', 
                            ...
                            \''.$trips->driver_id.'\', 
                            \''.$payout_method_data.'\',
                            \''.LOGIN_USER_TYPE.'/per_week_report/'.$trips->driver_id.'/'.request()->start_date.'/'.request()->end_date.'\', 
                            \''.$payout_text.'\', 
                            \'\', 
                            \'\', 
                            \''.date('Y-m-d',strtotime($trips->created_at)).'\'
                        )"> '.$payout_text.' </label>';
                        //used $trips->driver_payout  instead of $trips->driver_payout  
                }
