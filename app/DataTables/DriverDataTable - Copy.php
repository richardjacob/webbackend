...
class DriverDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            //     return date("h:i A d-m-Y", strtotime($users->created_at)); 
            // })
            ...
            ->addColumn('action', function ($users) {
                $edit = (LOGIN_USER_TYPE=='company' || auth('admin')->user()->can('update_driver')) ? '<a href="'.url(LOGIN_USER_TYPE.'/edit_driver/'.$users->id).'" class="btn btn-xs btn-primary" style="margin-bottom:5px;"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;' : '';
                $delete = (auth()->guard('company')->user()!=null || auth('admin')->user()->can('delete_driver')) ? '<a data-href="'.url(LOGIN_USER_TYPE.'/delete_driver/'.$users->id).'" class="btn btn-xs btn-primary" data-toggle="modal" data-target="#confirm-delete" style="margin-bottom:5px;"><i class="glyphicon glyphicon-trash"></i></a>&nbsp;':'';

                $remarks_id = DriverRemarks::where('driver_id', $users->id)->pluck('id')->first();

                if($remarks_id ==''){

                    $add_remarks = (LOGIN_USER_TYPE=='company' || LOGIN_USER_TYPE=='hub' || auth('admin')->user()->can('add_drivers_remarks')) ? '<a href="'.url(LOGIN_USER_TYPE.'/add_drivers_remarks/'.$users->id).'" class="btn btn-xs btn-success" style="margin-bottom:5px;"><i class="fa fa-comment"></i></a>&nbsp;' : '';
                }
                else{
                    $add_remarks = (LOGIN_USER_TYPE=='company' || LOGIN_USER_TYPE=='hub' || auth('admin')->user()->can('add_drivers_remarks')) ? '<a class="btn btn-xs btn-success" disabled style="margin-bottom:5px;"><i class="fa fa-comment"></i></a>&nbsp;' : '';
                }

                return $edit.$delete.$add_remarks;
            })
    ...
    public function query(User $model)
    {
        ...
        if (LOGIN_USER_TYPE=='company') {
            $users = $users->where('users.company_id', '21');//auth()->guard('company')->user()->id);
        }
