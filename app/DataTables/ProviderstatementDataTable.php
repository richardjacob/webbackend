...
class ProviderstatementDataTable extends DataTable
{
    ...
    public function query(User $model)
    {
        ...
        if (LOGIN_USER_TYPE=='company') {
            $drivers = $drivers->where('users.company_id',auth('company')->id());
        }
        if ($this->start_date !='') {
            $drivers = $drivers->where('users.created_at','>=', $this->start_date);
        }
        if ($this->end_date !='') {
            $drivers = $drivers->where('users.created_at','<=', $this->end_date);
        }
        if ($this->driver_id !='') {
            $drivers = $drivers->where('users.id', $this->driver_id);
        }
        return $drivers->groupBy('users.id');
    }
