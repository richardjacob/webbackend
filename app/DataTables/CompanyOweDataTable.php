...
class CompanyOweDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->addColumn('trip_ids', function ($owe) {
                //return '<div class="min_width text-primary trip_list_modal" data-id="'. $owe->driver_id.'" style="cursor:pointer">'.count(explode(',', $owe->trip_ids)).'</div>';

                return '<a href="../owe_trip_list/driver/'.$owe->driver_id.'">'.count(explode(',', $owe->trip_ids)).'</a>';
            })
            ...
            ->addColumn('action', function ($owe) {
                ...
                $paid_btn = '<form action="'.route('update_owe_payment').'" method="POST" onsubmit="return confirm(\'Are you sure to payment ?\');">
    ...
    public function query(User $model)
    {
        ...
        $owe = $model->join('trips', function($join) {
                ...
                ->select('trips.id as trip_id','users.id As id', 'trips.driver_id as driver_id', 'users.gender', 'users.first_name', 'trips.currency_code as currency_code',DB::raw("GROUP_CONCAT(trips.id) as trip_ids"),DB::raw('SUM(trips.total_fare) as total_fare'))
                ->groupBy('driver_id')
                ->get();
