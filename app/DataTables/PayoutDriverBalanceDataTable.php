...
class PayoutDriverBalanceDataTable extends DataTable
{
    ...
    public function dataTable($query)
        {
            return datatables()
                ...
                ->addColumn('action', function ($driver_balance) {
                    //if($driver_balance->status =='requested'){
                        if($driver_balance->company_id == '1'){
                            $payout_credentials = PayoutCredentials::where('user_id', $driver_balance->user_id)
                                                                    ->where('default','yes')
                                                                    ->first();
                        ...
                        if(is_object($payout_method)){
                            if($driver_balance->company_id == '1'){
                                ...
                                $preference = CompanyPayoutPreference::where('id', $payout_method->preference_id)
                                                                ->first();
                            }
                        ...
                        $account_number = @$preference->account_number;
                            ...
                            $payment_action='
                                ...
                                if($driver_balance->status =='requested'){
                                    $payment_action.='onclick="make_bonus_payout_modal(\''.$account_number.'\', 
                                        \''.$driver_balance->id.'\',
                                        \''.$driver_balance->bonus_id.'\',
                                        \''.$driver_balance->user_id.'\',
                                        \''.$driver_balance->amount.'\', 
                                        \''.$payout_method_data.'\',
                                        \''.LOGIN_USER_TYPE.'/payout/driver_balance\', 
                                        \''.$payout_text.'\'
                                    )" > '.$payout_text.' </label>'; 
                                }
                                else $payment_action.='disabled>'.$payout_text.' </label>';  
