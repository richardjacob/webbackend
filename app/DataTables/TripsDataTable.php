...
class TripsDataTable extends DataTable
{
        ...
        return datatables()
            ...
            ->addColumn('total_amount', function ($trips) {
                if (LOGIN_USER_TYPE=='company') {
                    return html_entity_decode(@$trips->currency->symbol).@(round($trips->driver_or_company_earning));
                }
                else{
                    return html_entity_decode(@$trips->currency->symbol).@(round($trips->admin_total_amount));
                }
            })
            ->addColumn('owe_amount', function ($trips) {
                if (LOGIN_USER_TYPE=='company') {
                    return html_entity_decode(@$trips->currency->symbol).@(round($trips->owe_amount));
                }
                else{
                    return html_entity_decode(@$trips->currency->symbol).@(round($trips->owe_amount));
                }
    ...
    {
        $trips =  $model->join('users as rider', function($join) {
