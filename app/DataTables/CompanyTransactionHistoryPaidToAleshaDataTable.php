...
class CompanyTransactionHistoryPaidToAleshaDataTable extends DataTable
{
    ...
    public function query()
    {
        $list = DB::table('driver_owe_amount_payments')
                    ->select(
                        ...
                        DB::raw('DATE_FORMAT(driver_owe_amount_payments.created_at, "%d-%b-%Y<br>%h:%i %p") as created_at')
                    )
        ...
        if($this->driver_id !='') $list = $list->where('driver_owe_amount_payments.user_id', $this->driver_id);
            ...
            if($this->start_date !='')  {
                ...
                $date1 =  $y.'-'.$m.'-'.$d;
                $list = $list->whereDate('driver_owe_amount_payments.created_at', '>=', $date1);
            }
            if($this->end_date !='')  {
                list($d, $m, $y) = explode('-', $this->end_date);
                $date2 =  $y.'-'.$m.'-'.$d;
                $list = $list->whereDate('driver_owe_amount_payments.created_at', '<=', $date2);
            }
        }
