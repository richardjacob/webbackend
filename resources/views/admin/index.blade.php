...
<div class="content-wrapper">
  ...
  @if(LOGIN_USER_TYPE=='company' || LOGIN_USER_TYPE=='hub' || auth('admin')->user()->can('manage_trips'))
  ...
  <section class="content">
    ...
    <div class="row">
      ...
      <section class="col-lg-12 connectedSortable">
        ...
        <!-- <select class="form-control" onchange="sales_graph(this.value)">        
          @foreach($select_option_array as $optionData)
            <option value='{!! @explode("|", $optionData[0])[1] !!}'
            @if(@$_GET['s'] == @explode("|", $optionData[0])[1]) selected="" @endif
            > {!! @explode("|", $optionData[0])[0] !!}</option>option>
          @endforeach
        </select> -->
        {!! Form::open(['method'=>'POST','url' => 'admin/dashboard', 'class' => 'form-horizontal','id'=>'dashboard']) !!}
        <select class="form-control" name="graph_type" onchange="this.form.submit()">  <!--  onchange="sales_graph(this.value)" -->   
          @foreach($select_option_array as $optionData)
            <option value='{!! $optionData !!}'
            @if($optionData == @$graph_type) selected="" @endif
...
<script type="text/javascript">
  // function sales_graph(val){
  //   document.getElementById("line-chart-data").value = val;
  //   //location.reload();  
  //    window.location.href = "dashboard?s="+val;  
  // }
