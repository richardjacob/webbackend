...
@endif
 @if(Auth::guard('company')->user()==null)
  ...
  <a href="{{ url('signin_company') }}" class="btn btn-for-rider">
  <!-- <a href="{{'//'.env('ADMIN_PANEL_SUB_DOMAIN'). '.' . env('DOMAIN').'/signin_company'}}" class="btn btn-for-rider"> -->
    
  <div class="block-context soft-small--right">{{trans('messages.home.company_signin')}}</div>
 <i class="fa fa-long-arrow-right icon icon_right-arrow-thin"></i>
  </a>
  </div>
@endif
