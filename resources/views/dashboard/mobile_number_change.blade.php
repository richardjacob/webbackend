...
<div class="modal otp-popup text-left poppayout fade" id="otp_popup" aria-hidden="false" style="" tabindex="-1">
	<div id="modal-add-otp-set-address" class="modal-content">
		...
		<div class="panel-body">
					...
					<div class="layout__item country-input" id="country">
						...
						<div class="select">
							<select name="country_code" tabindex="-1" id="mobile_country" class="square borderless--right">
								...
								<option value="{{$value->phone_code}}" {{ ($value->phone_code == (old('country_code')!=null? old('country_code') : '1')) ? 'selected' : ''  }} data-value="+{{ $value->phone_code}}"  data-name="{{ $value->short_name }}">{{ $value->long_name}}
								</option>
								@endif
								@endforeach
							</select> 
			...
			<div class="otp-field">
				<div class="otp-input">
					{!! Form::text('otp', '', ['id' => 'otp_input','class'=>'form-control','placeholder' => trans('messages.signup.otp')]) !!}
