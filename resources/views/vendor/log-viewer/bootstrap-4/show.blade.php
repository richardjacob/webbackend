...
<?php
    // $domain = "//";
    // $admin_sub_domain = env('ADMIN_PANEL_SUB_DOMAIN', '');
    // if( $admin_sub_domain !='') $domain.= $admin_sub_domain.".";
    // $domain.= env('DOMAIN')."/admin/all_logs/";
 
    $domain = "//";
    $domain.= $_SERVER['HTTP_HOST']."/admin/all_logs/";
    if (strpos($domain, 'admin.') !== false)  $admin_sub_domain = 'admin.';  
    else $admin_sub_domain = ''; 
?>
            ...
            <div class="card mb-4">
                ...
                <div class="list-group list-group-flush log-menu">
                    @foreach($log->menu() as $levelKey => $item)
                        @if($url = $item['url']) @endif

                        @if( $admin_sub_domain !='') 
                            @if($url = str_replace(['//www', '//'], ['//www'.$admin_sub_domain, '//'.$admin_sub_domain], $url)) @endif
                        @endif
                        ...
                        @else
                            <a href="{{ $url}}" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center level-{{ $levelKey }}{{ $level === $levelKey ? ' active' : ''}}">
                                <span class="level-name">{!! $item['icon'] !!} {{ $item['name'] }}</span>
        ...
        <div class="col-lg-10">
            ...
            <div class="card mb-4">
                <div class="card-header">
                    @lang('Log info') :
                    <!--
                        {{ route('log-viewer::logs.download', [$log->date]) }}
                        {{ route('log-viewer::logs.search', [$log->date, $level]) }}
                    -->
                    
                    <div class="group-btns pull-right">
                        <a href="{{url($domain.'logs/'.$log->date.'/download')}}" class="btn btn-sm btn-success">
                ...
                <div class="card-footer">
                    {{-- Search --}}
                    <form action="{{url($domain.'logs/'.$log->date.'/all/search')}}" method="GET">
...
@section('modals')
    {{-- DELETE MODAL --}}
    <!-- {{ route('log-viewer::logs.delete') }} -->
    <div id="delete-log-modal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <form id="delete-log-form" action="{{url($domain.'logs/delete')}}" method="POST">
